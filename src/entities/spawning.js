...
// spawns nothing). Caps come from config MOBS; passives count toward their
// cap only within DESPAWN_DISTANCE, because they never despawn (Phase 14) —
// herds left behind by a travelling player must not starve the cap forever.
//
// The per-mob registry (stats, drops, models) stays in entities/mobs.js per
// ARCHITECTURE.md; this module receives the type lists and a spawnAt
// callback from the mob manager.
...
export function createSpawner({ world, player, dayNight, hostileTypes, passiveTypes, mobs, spawnAt }) {
