...
- **DIRECTIONAL SUNLIGHT on every block face + a vignette** ("make the
  entire game look like shader... just look more lively"): the chunk
  shader derives each flat face's normal from screen derivatives (the
  meshes carry none — dFdx/dFdy of the world position, flipped for back
  faces) and brightens faces toward the light while faces away fall
  into shade — east faces glow in the morning, west faces at dusk, the
  swing crossing the whole world as the sun moves, and a fainter moon
  takes over at night (VISUAL.SUNLIGHT: STRENGTH 0.26, MOON_FACTOR
  0.35, direction = the cycle's lightDir). Scaled by each column's sky
  access so caves and interiors keep their baked look; 0 under fixed
  dimension skies; wind-swayed leaves get gently varying derivative
  normals for free, so canopies shimmer as they move. The post
...
- **THE DAYLIGHT POP** ("Shader has: Good sunlight. Whenever sun is
  ...
  S-curve, DAY_VIBRANCE 0.22 on saturation), all riding
  sunLevel-while-overworld-sky (uSunBoost) — full sun is bright, punchy
  and saturated, easing to the neutral grade through dusk and to zero at
  night and in the fixed-sky dimensions.
- **WAVING FOLIAGE** ("make those leaves look more lively... like
  shader minecraft — keep the texture"): the mesher bakes a per-vertex
  `wave` weight (one normalised byte — leaves ~0.55 on every vertex,
  cross-plane grass/flowers 0 at the root and 1 at the tip, everything
  else 0, kept in sync across every emit path by a zero-padding helper)
  and the chunk vertex shader sways weighted vertices through a
  world-space wind field: several incommensurate sines plus a slow
  travelling gust, phase continuous across blocks AND chunks so a
  canopy ripples as one body with no cracks. Textures untouched. Knobs
  in VISUAL.WIND (AMPLITUDE 0.09 blocks, SPEED); the wind clock lives
  in CHUNK_LIGHT_UNIFORMS.uWindTime, wrapped for float precision.
  Verified in node (41,392 leaf-weighted verts / 7,972 plant tips /
  zero anomalies over 25 chunks) and in the browser with a static
  camera: 9.1% of the tree-crown pixels move between frames a second
  apart while bare ground holds still; zero game console errors.
