...
- **A parallax TOP layer** (config TOP_LIFT 34): a colour-only plane
  riding above the base carrying the same field shrunken to its cores
  (dens^2), flat-lit bright via FLAT_SHEET, drawn between cirrus and the
  base colour pass. At a glancing view a single flat plane averages its
  interior shading into a flat sheet — the two-plane parallax is what
  finally made every puff read as having a white crown ABOVE a shaded
  base. Costs 11 noise evals per sky pixel (the cheap shading path).
