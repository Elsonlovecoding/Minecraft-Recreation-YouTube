...
- **Phase 27 follow-up: realistic clouds + moonlight** — the blocky slab
  ...
  at CLOUDS.HEIGHT whose fragment shader grows soft cumulus from 5-octave
  fbm value noise (world-anchored pattern + wrapped drift, weather-gate
  grouping with real clear sky, sun-probed self-shading, thin-edge
  brightening, silver linings near a low sun) plus a faint higher cirrus
  plane; all knobs in CLOUDS. Moonlight: a cool additive halo quad behind
  the pixel moon (`createMoonGlowTexture`, CELESTIAL.MOON_GLOW_*), a wide
  moonlit dome wash through the sky shader's glowBand uniform
  (MOON_SKY_GLOW_*), a brighter silver-blue night (LIGHTING.NIGHT_SKY_TINT,
  night SKY_DARKEN 10) and a moon glint lane on water
  (CELESTIAL.MOON_GLINT_LEVEL wired in main.js).
